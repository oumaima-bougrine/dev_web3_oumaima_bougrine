...
const CLIENT_COOLDOWN_SECONDS = 180
const CLIENT_COOLDOWN_STORAGE_KEY = 'dapp-vote--lastVoteTimestamp'

function getClientCooldownRemainingSeconds() {
  try {
    const stored = window.localStorage.getItem(CLIENT_COOLDOWN_STORAGE_KEY)
    if (!stored) return 0

    const lastVote = Number(stored)
    if (Number.isNaN(lastVote) || lastVote <= 0) return 0

    const elapsed = Math.floor((Date.now() - lastVote) / 1000)
    return Math.max(0, CLIENT_COOLDOWN_SECONDS - elapsed)
  } catch {
    return 0
  }
}

function setClientCooldownTimestamp() {
  try {
    window.localStorage.setItem(CLIENT_COOLDOWN_STORAGE_KEY, String(Date.now()))
  } catch {
  }
}
  ...
  const loadCandidates = useCallback(async (activeProvider) => {
    ...
    const list = await Promise.all(
      Array.from({ length: Number(count) }, async (_, index) => {
        const name = await contract.candidates(index);
        const voteCount = await contract.getVotes(index);
  ...
  const vote = useCallback(
    async (candidateIndex) => {
      ...
      try {
        ...
        const hasOnChainCooldown = typeof voteContract.getTimeUntilNextVote === 'function'

        if (hasOnChainCooldown) {
        ...
        if (!hasOnChainCooldown) {
          setClientCooldownTimestamp()
          setCooldownSeconds(CLIENT_COOLDOWN_SECONDS)
        }
